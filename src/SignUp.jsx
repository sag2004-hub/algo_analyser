...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      setSuccessMessage('Welcome to ALGO_Mania! Your account has been created.');
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row rounded-2xl overflow-hidden shadow-2xl bg-slate-800/50">
        ...
        <div className="w-full md:w-2/5 bg-gradient-to-br from-indigo-900 via-slate-900 to-purple-900 p-6 flex flex-col justify-center items-center">
          ...
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.5 }} className="text-center text-sm">
            <h2 className="text-xl font-bold text-white mb-3">Why Join Us?</h2>
            <ul className="text-indigo-200 space-y-2">
              <li className="flex items-center justify-center"><svg className="w-4 h-4 mr-2 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>Analyze algorithm complexity</li>
              <li className="flex items-center justify-center"><svg className="w-4 h-4 mr-2 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>Visualize data structures</li>
              <li className="flex items-center justify-center"><svg className="w-4 h-4 mr-2 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>Optimize your code</li>
            </ul>
          </motion.div>
        </div>
        ...
        <div className="w-full md:w-3/5 bg-slate-900/80 backdrop-blur-sm p-6 sm:p-8 flex flex-col justify-center relative">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-md mx-auto">
            <motion.div variants={itemVariants} className="text-center mb-6">
              ...
              <p className="text-slate-400 text-sm">And start your journey to code mastery.</p>
            </motion.div>
            ...
            <motion.form onSubmit={handleSignUp} className="space-y-3" noValidate>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <motion.div variants={itemVariants} whileHover={{ y: -2, transition: { type: 'spring', stiffness: 300 } }}>
                  <input 
                    ...
                    required 
                    className={`w-full px-3 py-2.5 bg-slate-800 border rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:border-transparent transition-colors duration-200 ${errors.firstName ? 'border-red-500 ring-red-500/50' : 'border-slate-700 focus:ring-emerald-500'}`} 
                ...
                <motion.div variants={itemVariants} whileHover={{ y: -2, transition: { type: 'spring', stiffness: 300 } }}>
                  <input 
                    ...
                    onChange={handleInputChange} 
                    required 
                    className={`w-full px-3 py-2.5 bg-slate-800 border rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:border-transparent transition-colors duration-200 ${errors.lastName ? 'border-red-500 ring-red-500/50' : 'border-slate-700 focus:ring-emerald-500'}`} 
              ...
              <motion.div variants={itemVariants} whileHover={{ y: -2, transition: { type: 'spring', stiffness: 300 } }}>
                <input 
                  ...
                  onChange={handleInputChange} 
                  required 
                  className={`w-full px-3 py-2.5 bg-slate-800 border rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:border-transparent transition-colors duration-200 ${errors.confirmPassword ? 'border-red-500 ring-red-500/50' : 'border-slate-700 focus:ring-emerald-500'}`} 
              ...
              <motion.div variants={itemVariants} className="flex items-start">
                <input 
                  ...
                  required 
                  id="terms" 
                  className="mt-0.5 h-4 w-4 rounded border-slate-600 text-emerald-500 focus:ring-emerald-500 bg-slate-800" 
                  disabled={loading}
                />
            ...
            <motion.div variants={itemVariants} className="mt-4 text-center">
              <p className="text-xs text-slate-400">
                Already have an account? <Link to="/signin" className="text-emerald-400 hover:underline font-medium">Sign in</Link>
              </p>
            </motion.div>
          </motion.div>
        </div>
